...
def get_project(project_id: str, timeout: int = 50) -> Dict[str, Any]:
    ...
    """
    return _get("/project.one", params={"projectId": project_id}, timeout=timeout)
...
def create_postgres(project_id: str, name: str, app_name: str, database_name: str, database_user: str, database_password: str, docker_image: str = "postgres:15") -> Any:
    """
    POST /postgres.create/ -> returns True for success or response. Return value forwarded.
    """
    payload = {
        "name": name,
        "appName": app_name,
...
def get_latest_postgres_entry_for_project(project_id: str) -> Optional[Dict[str, Any]]:
    """
    Convenience: fetch project via project.one and return the most-recent postgres entry (if any).
    Returns None when no postgres entry is present.
    """
    proj = get_project(project_id)
    # proj is expected to be a dict with 'postgres' list
    postgres_list = proj.get("postgres") or proj.get("postgreses") or []
    if not postgres_list:
        return None
    # prefer createdAt if available, otherwise first
    try:
        sorted_list = sorted(postgres_list, key=lambda x: x.get("createdAt") or "", reverse=True)
